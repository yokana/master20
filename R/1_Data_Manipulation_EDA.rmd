...
```{r echo=TRUE}
# Compute Raw average Scores in kindergarten
main_data <- 
...
reg_scores_gk <- 
  main_data_gk %>% filter(gkclasstype=="REGULAR CLASS" | gkclasstype=="REGULAR + AIDE CLASS", !is.na(gktmathss) & !is.na(gktreadss) & !is.na(gkwordskillss) ) %>%
    select(stdntid,gkclasstype,gktmathss,gktreadss, gkwordskillss, gkschid)
...
reg_scores_g1 <- 
  main_data_g1 %>% filter(g1classtype=="REGULAR CLASS" | g1classtype=="REGULAR + AIDE CLASS", !is.na(g1tmathss) & !is.na(g1treadss) & !is.na(g1wordskillss) )  %>%
    select(stdntid,g1classtype,g1tmathss,g1treadss, g1wordskillss, g1schid  )
...
reg_scores_g2 <- 
  main_data_g2 %>% filter(g2classtype=="REGULAR CLASS" | g2classtype=="REGULAR + AIDE CLASS", !is.na(g2tmathss) & !is.na(g2treadss) & !is.na(g2wordskillss) )  %>%
    select(stdntid,g2classtype,g2tmathss,g2treadss, g2wordskillss, g2schid )
...
reg_scores_g3 <- 
  main_data_g3 %>% filter(g3classtype=="REGULAR CLASS" | g3classtype=="REGULAR + AIDE CLASS", !is.na(g3tmathss) & !is.na(g3treadss) & !is.na(g3wordskillss) )  %>%
    select(stdntid,g3classtype,g3tmathss,g3treadss, g3wordskillss, g3schid  )
...
reg_scores_gk_sc <- 
  reg_scores_gk %>%
    group_by(gkschid) %>% 
      mutate(perc_maths_gk_sc=percent_rank(gktmathss), perc_read_gk_sc =percent_rank(gktreadss), perc_word_gk_sc=percent_rank(gkwordskillss)) %>%
        ungroup()
...
for (i in 1:nrow(small_scores_gk_sc)) {
  ...
  temp <- add_row(reg_scores_gk_sc, obs) %>%
    group_by(gkschid) %>%
    mutate(perc_maths_gk_sc=percent_rank(gktmathss), perc_read_gk_sc =percent_rank(gktreadss), perc_word_gk_sc=percent_rank(gkwordskillss)) %>% ungroup()
  # add observation to merged scores
...
# calculate average of the scores
scores_gk_sc <- 
  scores_gk_sc %>% mutate(perc_average_gk_sc = (perc_maths_gk_sc + perc_read_gk_sc + perc_word_gk_sc)/3)
...
reg_scores_g1_sc <- 
  reg_scores_g1 %>%
    group_by(g1schid) %>% 
      mutate(perc_maths_g1_sc=percent_rank(g1tmathss), perc_read_g1_sc =percent_rank(g1treadss), perc_word_g1_sc=percent_rank(g1wordskillss))  %>%
        ungroup()
...
# calculate average of the scores
scores_g1_sc <- 
  scores_g1_sc %>% mutate(perc_average_g1_sc = (perc_maths_g1_sc + perc_read_g1_sc + perc_word_g1_sc)/3)
...
reg_scores_g2_sc <- 
  reg_scores_g2 %>%
    group_by(g2schid) %>% 
      mutate(perc_maths_g2_sc=percent_rank(g2tmathss), perc_read_g2_sc =percent_rank(g2treadss), perc_word_g2_sc=percent_rank(g2wordskillss))  %>%
        ungroup()
...
# calculate average of the scores
scores_g2_sc <- 
  scores_g2_sc %>% mutate(perc_average_g2_sc = (perc_maths_g2_sc + perc_read_g2_sc + perc_word_g2_sc)/3)
...
reg_scores_g3_sc <- 
  reg_scores_g3 %>%
    group_by(g3schid) %>% 
      mutate(perc_maths_g3_sc=percent_rank(g3tmathss), perc_read_g3_sc =percent_rank(g3treadss), perc_word_g3_sc=percent_rank(g3wordskillss))  %>%
        ungroup()
...
# calculate average of the scores
scores_g3_sc <- 
  scores_g3_sc %>% mutate(perc_average_g3_sc = (perc_maths_g3_sc + perc_read_g3_sc + perc_word_g3_sc)/3)
    ...
    full_join(scores_g1_sc, by = "stdntid") %>% 
      full_join(scores_g2_sc, by = "stdntid") %>% 
        full_join(scores_g3_sc, by = "stdntid") %>% 
          select(stdntid, perc_average_g3_sc, perc_maths_g3_sc, perc_read_g3_sc, perc_word_g3_sc, perc_average_g2_sc, perc_maths_g2_sc, perc_read_g2_sc, perc_word_g2_sc, perc_average_g1_sc, perc_maths_g1_sc, perc_read_g1_sc, perc_word_g1_sc,perc_average_gk_sc, perc_maths_gk_sc, perc_read_gk_sc, perc_word_gk_sc)
# we get a problem here, because covariates are in different categories, better make a dataset with new variables and merge them with the main_data
main_data <- 
