...
The **grid of evaluation points** is specified as $t=\left(t_{1}, \ldots, t_{T}\right)^{\top}$ with $T$ being the number of points. Since our constructed response is a density on $\mathscr{I}$ and we evaluate the density on the same grid 
...
In the context of additive models with smooth functions of covariates, we usually deal with an identifiability problem that arises because we can always add a constant to one function and substract it from another and still get exactly the same model fit. To avoid this, we impose a constraint on the model matrix of our covariates. In the general case, it is enough to constraint each additive effects to sum up to zero. But as Scheipl et al. (2015) pointed out, this can cause problems when we are dealing with functional responses evaluated at a vector of observation points. In this context, it is still possible to run into an identificiation problem, since it is now possible to add the sample mean of a smooth effect for one observation point to the *functional intercept* and substract it from the respective effect with the same result. That means in theory we could shift the functional intercept for a specific evaluation point without changing the model fit. For this reason it makes sense to use a constraint that constraints the mean effect to be zero at each observation point t:
