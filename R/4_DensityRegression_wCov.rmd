...
```{r echo=TRUE}
mod2 <- FDboost(Den_gk ~ 1 + bols(ClassType, intercept = T, df = 4) + bolsc(School, intercept = T, df = 4),
               timeformula = ~ bbs(evPoints),
               offset = "scalar", data = star)  

plot(mod2, which = 2, main = "Linear effects for schools", ylim = c(-0.1,0.1))
```

Model with continious Covariates
